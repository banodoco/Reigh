...
serve(async (req) => {
  ...
  if (!isWelcomeBonus && !isServiceRole) {
    return new Response("Service role access required for admin grants", { status: 403 });
  }
  ...
  try {
    ...
    if (isWelcomeBonus) {
      ...
      if (!isServiceRole && currentUserId !== userId) {
        return new Response("Users can only request welcome bonus for themselves", { status: 403 });
      }
      ...
      if (userError) {
        ...
        return new Response(`User not found: ${userError.message}`, { status: 400 });
      }

      if (user.given_credits) {
        return new Response("Welcome bonus already given to this user", { status: 400 });
      }
      ...
      if (ledgerError) {
        ...
        return new Response(`Failed to grant welcome credits: ${ledgerError.message}`, { status: 500 });
      }
