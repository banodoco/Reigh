-- Fix PAT claim function - remove LEFT JOIN that conflicts with FOR UPDATE
...
BEGIN
  ...
  -- Split dependency check to avoid LEFT JOIN with FOR UPDATE
  ...
  claimed_task AS (
    ...
    WHERE tasks.id = (
      -- Subquery to find the oldest eligible task without LEFT JOIN
      SELECT t.id
      FROM tasks t
...
COMMENT ON FUNCTION claim_next_task_user_pat IS 'PAT-friendly version: Atomically claims next eligible task for specific user without credits or run_type constraints. Fixed outer join issue with FOR UPDATE.';
