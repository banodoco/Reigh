-- Add PAT-friendly version of claim function that bypasses credits and run_type constraints
-- PAT users should be able to claim their tasks regardless of credits or run_type

-- =============================================================================
-- claim_next_task_user_pat - PAT version without credits/run_type constraints
-- =============================================================================
...
BEGIN
  ...
  ready_tasks AS (
    -- Find tasks that meet dependency criteria (NO run_type filter for PAT)
    SELECT 
      t.id,
      t.params,
      t.task_type,
...
COMMENT ON FUNCTION claim_next_task_user_pat IS 'PAT-friendly version: Atomically claims next eligible task for specific user without credits or run_type constraints. Excludes orchestrator tasks from concurrency limits.';
