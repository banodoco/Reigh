...
-- First, add the new column with decimal type (storing cents per second)
ALTER TABLE task_cost_configs 
ADD COLUMN base_cost_per_second decimal(10,6) NOT NULL DEFAULT 0.0278;

-- Update all existing records to use the new standardized cost (0.0278 cents per second = $0.000278 per second)
