-- Fix thumbnail generation trigger to work without HTTP extension
-- Instead of calling edge function from trigger, we'll add a status field and let the client handle it

-- Add a thumbnail_status field to track thumbnail generation
ALTER TABLE generations ADD COLUMN IF NOT EXISTS thumbnail_status text DEFAULT 'pending';

-- Create index for efficient queries
CREATE INDEX IF NOT EXISTS idx_generations_thumbnail_status ON generations(thumbnail_status) 
WHERE thumbnail_status = 'pending';

-- Update the trigger to just mark for thumbnail generation instead of calling HTTP
    ...
    IF NEW.status = 'Complete'::task_status
       ...
       AND task_category = 'generation' THEN
        ...
        BEGIN
            ...
            IF extracted_shot_id IS NULL THEN
                ...
                ELSE -- For other generation task types (e.g., image-generation, magic-edit)
                    ...
                    -- Also check for add_in_position at top level (for magic-edit tasks)
        ...
        BEGIN
            ...
            RAISE LOG '[ProcessTask] Extracted thumbnail_url for % task %: %', NEW.task_type, NEW.id, COALESCE(thumbnail_url, 'none');
        EXCEPTION WHEN OTHERS THEN
            thumbnail_url := NULL;
            RAISE LOG '[ProcessTask] Error extracting thumbnail_url for % task %: %', NEW.task_type, NEW.id, SQLERRM;
        END;
        ...
        -- Insert the generation record with thumbnail status
        INSERT INTO generations (
            ...
            thumbnail_url,
            CASE 
                WHEN generation_type = 'image' AND thumbnail_url IS NULL THEN 'pending'
                ELSE 'completed'
            END,
            NOW()
        );
        ...
        RAISE LOG '[ProcessTask] Created generation % for % task % with category: %, tool_type: %, thumbnail_status: %, add_in_position: %', 
            new_generation_id, NEW.task_type, NEW.id, task_category, task_tool_type, 
            CASE WHEN generation_type = 'image' AND thumbnail_url IS NULL THEN 'pending' ELSE 'completed' END,
            add_in_position;
    ELSE
...
-- Log confirmation
SELECT 'Updated trigger to work without HTTP extension - thumbnails will be generated by client polling' as status;
