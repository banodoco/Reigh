-- Force complete recreation of apply_timeline_frames function
-- Drop all versions and recreate to ensure the fix is applied

-- Drop all possible versions of the function
DROP FUNCTION IF EXISTS apply_timeline_frames(uuid, jsonb, boolean);
DROP FUNCTION IF EXISTS apply_timeline_frames(uuid, jsonb);
DROP FUNCTION IF EXISTS apply_timeline_frames(uuid, text, boolean);
DROP FUNCTION IF EXISTS apply_timeline_frames(uuid, text);

-- Recreate with proper column qualifiers
  ...
  IF p_update_positions THEN
    WITH ordered_items AS (
      SELECT 
        ...
        ROW_NUMBER() OVER (
          ORDER BY 
            sg.timeline_frame NULLS LAST,  -- FIXED: Properly qualified
            sg.created_at ASC, 
            sg.generation_id ASC
        ) - 1 as new_position
      FROM shot_generations sg  -- FIXED: Proper table alias
      WHERE sg.shot_id = p_shot_id
    )
    UPDATE shot_generations sg
    SET 
...
SELECT 'Successfully recreated apply_timeline_frames function with proper column qualifiers' as status;
