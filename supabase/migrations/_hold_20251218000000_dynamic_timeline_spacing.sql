-- Update add_generation_to_shot to use dynamic average spacing
-- Instead of hardcoded +50, calculates the average spacing between existing items
-- and uses that for new items (matching client-side behavior)
...
DECLARE
  ...
  max_frame integer;
  avg_spacing integer;
  DEFAULT_SPACING constant integer := 50;
...
BEGIN
  ...
  IF p_with_position THEN
    ...
    ELSE
      ...
      avg_spacing := GREATEST(MIN_SPACING, (max_frame - min_frame) / (item_count - 1));
    END IF;
...
COMMENT ON FUNCTION add_generation_to_shot IS 'Adds a generation to a shot with dynamic timeline spacing. Calculates average spacing from existing items (min 10, default 50). Always creates a new record, allowing duplicates.';
