-- Add image_edit task type support to generation creation trigger
-- This allows magic edit tasks to automatically create generation records when completed
-- Using the standard image_edit task type instead of the custom qwen_image_edit
...
BEGIN
    ...
    IF NEW.status = 'Complete'::task_status 
       ...
       AND NEW.task_type IN ('travel_stitch', 'single_image', 'image_edit') THEN
        ...
        -- Process image_edit tasks (Magic Edit)
        ELSIF NEW.task_type = 'image_edit' THEN
            generation_type := 'image';
            
            -- SAFE: Extract shot_id if present with exception handling
            -- image_edit tasks may have shot_id directly in params or nested
            BEGIN
                shot_id := (normalized_params->>'shot_id')::uuid;
            EXCEPTION 
                WHEN invalid_text_representation OR data_exception THEN
                    shot_id := NULL; -- Continue without shot linking
                    RAISE LOG '[ProcessTask] Invalid shot_id format in image_edit task %, continuing without shot link', NEW.id;
            END;
            ...
            -- Validate required fields
            IF output_location IS NULL OR NEW.project_id IS NULL THEN
                RAISE LOG '[ProcessTask] Missing critical data for image_edit task %: output_location=%, project_id=%', 
                    NEW.id, output_location, NEW.project_id;
                RETURN NEW;
            END IF;
            
            -- Build generation params for image edit
            generation_params := jsonb_build_object(
                'type', 'image_edit',
                'projectId', NEW.project_id,
                'outputLocation', output_location,
            ...
            IF thumbnail_url IS NOT NULL THEN
                generation_params := generation_params || jsonb_build_object('thumbnailUrl', thumbnail_url);
                RAISE LOG '[ProcessTask] Found thumbnail_url for image_edit task %: %', NEW.id, thumbnail_url;
            END IF;
        END IF;
...
-- Process any existing completed image_edit tasks that haven't created generations yet
UPDATE tasks 
SET updated_at = NOW()
WHERE status = 'Complete'::task_status 
    AND generation_created = FALSE
    AND task_type = 'image_edit';

-- Confirm the migration was applied
SELECT 'Added image_edit support to create_generation_on_task_complete function' as status;
