-- Fix generation trigger to properly set generation_created flag and include tool_type in params
-- Issues fixed:
-- 1. generation_created flag not being set (AFTER UPDATE trigger can't modify NEW)
-- 2. tool_type missing from generation params JSONB
...
-- Log confirmation
SELECT 'Fixed generation trigger: now properly sets generation_created flag and includes tool_type in params' as status;
