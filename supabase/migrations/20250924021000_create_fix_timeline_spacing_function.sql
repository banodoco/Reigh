-- Create reusable function to fix timeline violations for a specific shot
-- Implements violation-only fix strategy that preserves spacing and only fixes problems:
-- 1. First frame normalization (must be at 0)
-- 2. Distance violations (gaps too large for context frames)  
-- 3. Duplicate violations (same timeline_frame)
...
BEGIN
  ...
  SELECT sg.id, sg.generation_id, sg.timeline_frame, sg.metadata, sg.created_at,
  ...
  IF first_frame != 0 THEN
    ...
    LOOP
      ...
      SET 
        ...
        updated_at = NOW()
      WHERE shot_generations.id = item_record.id;
  ...
  LOOP
    ...
    IF item_record.timeline_frame > max_allowed_frame THEN
      ...
      WHERE shot_generations.id IN (
        SELECT t.id FROM temp_eligible_items t 
