-- Fix the claim function RETURNING clause issue
-- The problem is trying to return CTE columns in UPDATE RETURNING clause
-- Revert to the working pattern that uses variables

BEGIN;

-- Fix claim_next_task_service_role by reverting to working RETURNING pattern
