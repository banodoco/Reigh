-- Update RPC to also clear generation display fields when demoting
-- This replaces the trigger-based approach which had race condition issues
-- during normal primary switches.
...
BEGIN
  ...
  LOOP
    ...
    -- If they don't match, demote the variant AND clear the generation
    IF stored_gen_id IS NOT NULL AND current_gen_id IS DISTINCT FROM stored_gen_id THEN
      -- Step 1: Demote the variant
      UPDATE generation_variants
      SET is_primary = false
      WHERE id = video_record.variant_id;

      -- Step 2: Clear the generation's display fields
      -- This ensures the UI shows the slot as empty/needing regeneration
      -- We do this here instead of via trigger to avoid race conditions
      -- during normal primary switches (where another variant is being promoted)
      UPDATE generations
      SET
        location = NULL,
        thumbnail_url = NULL,
...
-- Also drop the trigger if it was created (in case migration 20250128000002 ran before this fix)
DROP TRIGGER IF EXISTS trg_handle_variant_demotion ON generation_variants;
DROP FUNCTION IF EXISTS handle_variant_demotion();
