-- Fix trigger to process ANY completed task that doesn't have a generation yet
-- This handles both new completions and existing completed tasks

-- Drop the existing trigger
DROP TRIGGER IF EXISTS trigger_create_generation_on_task_complete ON tasks;

-- Update the function to remove the OLD.status check
...
BEGIN
    ...
    -- Removed the OLD.status != 'Complete' check to handle existing completed tasks
...
'Creates generation records for ANY completed task without a generation (not just status transitions). This handles both new completions and existing completed tasks.';

-- Now trigger generation creation for all existing completed tasks
-- This will run the trigger for each task that should have a generation
UPDATE tasks 
SET updated_at = NOW()
