-- Fix tool_type mapping for wan_2_2_t2i tasks to appear in image-generation gallery filter
-- The generation trigger currently maps single_image -> 'image-generation' but wan_2_2_t2i -> 'unknown'
-- This causes new generations to be excluded from gallery filters
    ...
    IF NEW.status = 'Complete'::task_status 
       ...
       AND task_category = 'generation' THEN
        ...
        ELSE
            ...
            -- ✅ FIXED: Map wan_2_2_t2i to 'image-generation' like single_image
            generation_params := jsonb_build_object(
                'type', NEW.task_type,
                'projectId', NEW.project_id,
                'outputLocation', output_location,
                'originalParams', normalized_params,
                'tool_type', CASE 
                    WHEN NEW.task_type IN ('single_image', 'wan_2_2_t2i') THEN 'image-generation'
        ...
        RAISE LOG '[ProcessTask] Created generation % for % task % with tool_type: % and thumbnail_url: %', 
            new_generation_id, NEW.task_type, NEW.id, 
            CASE 
                WHEN NEW.task_type IN ('single_image', 'wan_2_2_t2i') THEN 'image-generation'
                WHEN NEW.task_type IN ('image_edit', 'qwen_image_edit', 'magic_edit') THEN 'magic-edit'
                WHEN NEW.task_type LIKE '%edit_travel%' THEN 'edit-travel'
...
-- Update any existing wan_2_2_t2i generations that have tool_type = 'unknown'
UPDATE generations 
SET params = params || jsonb_build_object('tool_type', 'image-generation')
WHERE params->'tool_type' = '"unknown"'
  AND EXISTS (
    SELECT 1 FROM tasks t 
    WHERE t.id = ANY(ARRAY(SELECT (jsonb_array_elements_text(generations.tasks))::uuid))
    AND t.task_type = 'wan_2_2_t2i'
  );

-- Confirm the migration was applied
SELECT 'Fixed wan_2_2_t2i tool_type mapping to image-generation for gallery filtering' as status;
