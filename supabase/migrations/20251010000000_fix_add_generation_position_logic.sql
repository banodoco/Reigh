-- Fix add_generation_to_shot to use same position calculation as image upload
-- Issue: The function was filtering out user-positioned items when calculating next position,
-- causing new items to be added at position 0 when all existing items were user-positioned.
-- Solution: Match the image upload logic - consider ALL items with valid timeline_frame values.
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF p_with_position AND existing_record.timeline_frame IS NULL THEN
      ...
      -- Use same logic as image upload: look at ALL items with valid timeline_frame
      ...
      WHERE shot_generations.id = existing_record.id;
...
COMMENT ON FUNCTION add_generation_to_shot IS 'Add a generation to a shot with optional timeline_frame positioning. Uses same logic as image upload: finds max timeline_frame across ALL positioned items and adds 50.';
