-- Update create_generation_on_task_complete function to extract thumbnail_url
-- This adds support for extracting thumbnails from task parameters and storing them in generations
    ...
    IF NEW.status = 'Complete'::task_status 
       ...
       AND NEW.task_type IN ('travel_stitch', 'single_image') THEN
        ...
        INSERT INTO generations (
            ...
            generation_type,
            NEW.project_id,
            thumbnail_url,  -- Store extracted thumbnail URL
            NOW()
        );
...
SELECT 'create_generation_on_task_complete function updated with thumbnail support' as status;
