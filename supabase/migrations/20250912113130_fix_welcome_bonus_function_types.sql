-- Fix type mismatch in check_and_grant_welcome_bonus function
-- The credits column is numeric(10,3) but the function was returning integer
-- We need to drop and recreate since PostgreSQL won't let us change return types

DROP FUNCTION IF EXISTS check_and_grant_welcome_bonus();

CREATE FUNCTION check_and_grant_welcome_bonus()
RETURNS TABLE(
  granted boolean,
...
COMMENT ON FUNCTION check_and_grant_welcome_bonus() IS 'Atomically checks and grants welcome bonus to new users. Returns status and updated credits balance. Fixed to return numeric(10,3) for credits_balance.';
