-- Fix initialize_timeline_frames_for_shot to start timeline frames at 0 instead of 50
-- This makes the timeline start at the beginning without unnecessary padding
...
DECLARE
  ...
  next_frame integer := 0; -- Start at frame 0 instead of 50
BEGIN
  ...
  SELECT COALESCE(MAX(timeline_frame), -1) INTO max_existing_frame
  FROM shot_generations
  WHERE shot_id = p_shot_id AND timeline_frame IS NOT NULL;

  -- Set starting frame to max existing + frame_spacing, or 0 if no existing frames
  next_frame := GREATEST(max_existing_frame + p_frame_spacing, 0);
...
-- Verify the fix
SELECT 'Fixed initialize_timeline_frames_for_shot to start timeline frames at 0 instead of 50' as status;
