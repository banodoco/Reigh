-- Include segment children in RPC output
-- Child segment videos are NOT in shot_generations - they're children of parent video generations
-- The hook needs these children to display in the output video strip
...
BEGIN
  ...
  -- 8. Return data - settings is RAW from database, no transformation
  RETURN jsonb_build_object(
    'shot_id', share_record.shot_id,
    'shot_name', shot_record.name,
    'generation', shared_generation,
    'images', all_generations,
    'settings', COALESCE(shot_record.settings->'travel-between-images', '{}'::jsonb),
