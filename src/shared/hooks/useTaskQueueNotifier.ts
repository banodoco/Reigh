...
import { useCreateTask, useListTasks } from './useTasks';
...
export const useTaskQueueNotifier = ({
  ...
  const { data: currentTasks } = useListTasks({ projectId });
  ...
  const enqueueTasks = useCallback(
    async (payloads: TaskPayload[]): Promise<void> => {
      ...
      const initialCount = currentTasks?.length ?? 0;
    ...
    [projectId, createTaskAsync, currentTasks?.length, queryClient, handleSuccess]
  );
  ...
  useEffect(() => {
    console.log(`[${Date.now()}] [TaskQueueNotifier] Watcher effect running:`, {
      ...
      targetTotal,
      currentTasksLength: currentTasks?.length,
      currentTasksLoaded: !!currentTasks,
    });
    ...
    console.log(`[${Date.now()}] [TaskQueueNotifier] Watcher: Checking if target reached:`, {
      ...
      target: targetTotal,
      reached: currentTasks.length >= targetTotal,
    });

    if (currentTasks.length >= targetTotal) {
      console.log(`[${Date.now()}] [TaskQueueNotifier] 🎉 Target reached via fallback watcher`);
      handleSuccess(); // Use same success handler
    }
  }, [currentTasks, isEnqueuing, targetTotal, handleSuccess]);
