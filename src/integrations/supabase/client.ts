...
console.error('[ReconnectionIssue] 🔥 CLIENT.TS FILE LOADED AT:', new Date().toISOString());
...
console.error('[DevEnvCheck] 🔍 Environment Detection:', {
...
console.error('[ReconnectionIssue] 🚨 SUPABASE CLIENT INITIALIZATION:', {
  url: SUPABASE_URL,
...
try {
  console.error('[ReconnectionIssue] 🔥 ATTEMPTING TO CREATE SUPABASE CLIENT...');
    ...
    global: {
      fetch: (url: string, options: RequestInit = {}) => {
        ...
        const timeoutId = setTimeout(() => {
          console.error('[ReconnectionIssue] Aborting request due to timeout:', { url, timeoutMs });
          controller.abort();
        }, timeoutMs);
  ...
  console.error('[ReconnectionIssue] 🔥 SUPABASE CLIENT CREATED SUCCESSFULLY!');
} catch (error: any) {
  console.error('[ReconnectionIssue] ❌ SUPABASE CLIENT CREATION FAILED:', error);
  console.error('[DeepDebug] 🔍 CLIENT CREATION ERROR DETAILS:', { errorMessage: error?.message, errorType: typeof error, errorConstructor: error?.constructor?.name, errorStack: error?.stack?.split('\n').slice(0, 5), timestamp: Date.now() });
  throw error;
}
...
console.error('[ReconnectionIssue] 🔥 SUPABASE CLIENT EXISTS:', !!supabase);
console.error('[ReconnectionIssue] 🔥 REALTIME EXISTS:', !!(supabase as any)?.realtime);
console.error('[ReconnectionIssue] 🔥 SOCKET EXISTS:', !!(supabase as any)?.realtime?.socket);
console.error('[ReconnectionIssue] 🔥 SOCKET STATE:', (supabase as any)?.realtime?.socket?.readyState);
