...
console.info('[DevEnvCheck] 🔍 Environment Detection:', {
...
console.info('[ReconnectionIssue] 🚨 SUPABASE CLIENT INITIALIZATION:', {
  url: SUPABASE_URL,
...
try {
  console.info('[ReconnectionIssue] 🔥 ATTEMPTING TO CREATE SUPABASE CLIENT...');
  ...
  console.info('[ReconnectionIssue] 🔥 SUPABASE CLIENT CREATED SUCCESSFULLY!');
} catch (error: any) {
  console.error('[ReconnectionIssue] ❌ SUPABASE CLIENT CREATION FAILED:', error);
  console.debug('[DeepDebug] 🔍 CLIENT CREATION ERROR DETAILS:', { errorMessage: error?.message, errorType: typeof error, errorConstructor: error?.constructor?.name, errorStack: error?.stack?.split('\n').slice(0, 5), timestamp: Date.now() });
  throw error;
}
...
console.info('[ReconnectionIssue] 🔥 SUPABASE CLIENT EXISTS:', !!supabase);
console.info('[ReconnectionIssue] 🔥 REALTIME EXISTS:', !!(supabase as any)?.realtime);
console.info('[ReconnectionIssue] 🔥 SOCKET EXISTS:', !!(supabase as any)?.realtime?.socket);
console.info('[ReconnectionIssue] 🔥 SOCKET STATE:', (supabase as any)?.realtime?.socket?.readyState);
